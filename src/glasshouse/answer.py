...
def build_prompt(question: str, docs: Iterable, people: Iterable) -> str:
    """Lay out the evidence, identities first."""
    lines: list[str] = []
...
def write(question: str, docs, people, model: str | None = None) -> Written:
    """Answer in one shot."""
    response = _client().chat(
        model=model or ADJUDICATION_MODEL,
        messages=[
            {"role": "system", "content": SYSTEM},
            {"role": "user", "content": build_prompt(question, docs, people)},
        ],
...
def write_streaming(question: str, docs, people, model: str | None = None) -> Iterator[dict]:
